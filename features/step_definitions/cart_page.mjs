...
/**
 * The user completes the checkout process by entering first name "<first_name>", last name "<last_name>", and zip code "<zip_code>" and navigates back to the products page
 ...
 * @path=/cart
 */
async function the_user_completes_the_checkout_process_by_entering_first_name_first_name_last_name_last_name_and_zip_code_zip_code_and_navigates_back_to_the_products_page(
  _first_name,
  _last_name,
  _zip_code
) {
  // source: recorder
  // implemented_at: 2025-04-28T11:29:56.248Z
  const _params = { _first_name, _last_name, _zip_code };
  // Click on Checkout button
  await context.web.click(elements["button_checkout"], _params, null, this);
  ...
  // Press Tab
  await context.web.clickType(elements["textbox_first_name"], "Tab", null, _params, { press: true }, this);
  // Fill Last name textbox with "_last_name"
  await context.web.clickType(elements["textbox_last_name"], _last_name, false, _params, null, this);
  // Press Tab
  await context.web.clickType(elements["textbox_last_name"], "Tab", null, _params, { press: true }, this);
  // Fill Zip/Postal code textbox with "_zip_code"
  await context.web.clickType(elements["textbox_zip_postal_code"], _zip_code, false, _params, null, this);
  // Click on Checkout button
  await context.web.click(elements["button_checkout_1"], _params, null, this);
  // Click on Back home button
  await context.web.click(elements["button_back_home"], _params, null, this);
}

When(
  "The user completes the checkout process by entering first name {string}, last name {string}, and zip code {string} and navigates back to the products page",
  { timeout: 480000 },
  the_user_completes_the_checkout_process_by_entering_first_name_first_name_last_name_last_name_and_zip_code_zip_code_and_navigates_back_to_the_products_page
);
